...
function Top() {
  const { openCharacteristics, setOpenDetail } = useContext(GlobalContext);
    ...
    return (
      <div className="noTouch h-3/5 w-full bg-cover bg-top bg-no-repeat rounded-3xl" style={{ backgroundImage: `url(${openCharacteristics[0]})` }}>
          <p className="noTouch w-full h-1/6 font-bold text-3xl text-center bg-white bg-opacity-20 rounded-t-3xl">{openCharacteristics[1]}</p>
