...
function MyOrderCard (props) {
    const { adjustCount } = useContext(GlobalContext);
    ...
    return (
        <div className="noTouch flex flex-row bg-black bg-opacity-10 rounded-xl w-3/5 h-28 mt-10">
            ...
            <div className="font-bold text-xl h-full w-1/6 flex flex-row justify-center items-center"><span>Units: {props.count}</span><div className="font-bold text-xl h-full flex flex-col justify-center items-center ml-3"><span className="cursor-pointer" onClick={() => {adjustCount(props.id, '+')}}>+</span><span className="cursor-pointer" onClick={() => {adjustCount(props.id, '-')}}>-</span></div></div>
